...
def train_autoencoder(noisy_data, ideal_data, batch_size=32, epochs=50, learning_rate=0.001):
    ...
    model.to(device)  # 将模型迁移到设备（GPU 或 CPU）

    # 转换数据为torch tensor并迁移到设备
    noisy_data = torch.tensor(noisy_data, dtype=torch.float32).to(device)
    ideal_data = torch.tensor(ideal_data, dtype=torch.float32).to(device)
...
def denoise_data(model, noisy_data):
    ...
    model.eval()
    noisy_data_tensor = torch.tensor(noisy_data, dtype=torch.float32).to(device)  # 将数据迁移到设备
    with torch.no_grad():
